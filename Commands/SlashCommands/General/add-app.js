...
import { UrlAddAppShort } from '../../../Resources/hyperlinks.js';
...
export const SlashCommand = {
     ...
     * @type {String}
     */
    description: "Shows the link for adding HomeCord to your own Servers",

    /** Command's Localised Descriptions
     * @type {import('discord-api-types/v10').LocalizationMap}
     */
    localizedDescriptions: {
        'en-GB': 'Shows the link for adding HomeCord to your own Servers',
        'en-US': 'Shows the link for adding HomeCord to your own Servers'
    },
    ...
    async executeCommand(interaction, api, interactionUser, usedCommandName) {
        ...
        const LinkButton = new ActionRowBuilder().addComponents([
            new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel(localize(interaction.locale, 'INVITE_COMMAND_BUTTON_LABEL')).setURL(UrlAddAppShort)
        ]);

        await api.interactions.reply(interaction.id, interaction.token, { flags: MessageFlags.Ephemeral, components: [LinkButton], content: localize(interaction.locale, 'INVITE_COMMAND_RESPONSE') });
