        ...
        if ( Timestamps.has(selectInteraction.user.id) )
        {
            ...
            if ( Now < ExpirationTime )
            {
                ...
                if ( timeLeft >= 60 && timeLeft < 3600 )
                {
                    ...
                    await selectInteraction.reply({ ephemeral: true, content: localize(selectInteraction.locale, 'SELECT_MENU_ERROR_COOLDOWN_MINUTES', timeLeft.toFixed(1)) });
                    return;
                }
                // HOURS
                else if ( timeLeft >= 3600 && timeLeft < 86400 )
                {
                    ...
                    await selectInteraction.reply({ ephemeral: true, content: localize(selectInteraction.locale, 'SELECT_MENU_ERROR_COOLDOWN_HOURS', timeLeft.toFixed(1)) });
                    return;
                }
                // DAYS
                else if ( timeLeft >= 86400 && timeLeft < 2.628e+6 )
                {
                    ...
                    await selectInteraction.reply({ ephemeral: true, content: localize(selectInteraction.locale, 'SELECT_MENU_ERROR_COOLDOWN_DAYS', timeLeft.toFixed(1)) });
                    return;
                }
                // MONTHS
                else if ( timeLeft >= 2.628e+6 )
                {
                    ...
                    await selectInteraction.reply({ ephemeral: true, content: localize(selectInteraction.locale, 'SELECT_MENU_ERROR_COOLDOWN_MONTHS', timeLeft.toFixed(1)) });
                    return;
                }
                // SECONDS
                else
                {
                    await selectInteraction.reply({ ephemeral: true, content: localize(selectInteraction.locale, 'SELECT_MENU_ERROR_COOLDOWN_SECONDS', timeLeft.toFixed(1)) });
                    return;
                }
            }
        }
