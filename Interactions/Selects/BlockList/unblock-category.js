...
module.exports = {
    ...
    Name: "unblock-category",

    // Select's Description
    Description: `Removes the selected Categories from the Server's Block List`,
    ...
    async execute(interaction)
    {
        ...
        .then(async existingEntries => {
            ...
            if ( existingEntries.length >= 1 )
            {
                ...
                if ( channelsInDb.size < 1 ) { await interaction.editReply({ components: [], content: localize(interaction.locale, 'UNBLOCK_COMMAND_ALL_CATEGORIES_NOT_BLOCKED') }); return; }
                ...
                .then(async oldDocuments => {
                    ...
                    // ACK to User
                    await interaction.editReply({ components: [], content: `${localize(interaction.locale, 'UNBLOCK_COMMAND_CATEGORY_SUCCESSFUL', channelsInDb.map(channelId => `<#${channelId}>`).join(', '))}${inputChannels.size > 0 ? `\n\n${localize(interaction.locale, 'UNBLOCK_COMMAND_CATEGORY_NOT_BLOCKED', inputChannels.map(channel => `<#${channel.id}>`).join(', '))}` : ''}` });
                ...
                .catch(async err => {
                    await LogError(err);
                    await interaction.editReply({ components: [], content: localize(interaction.locale, 'UNBLOCK_COMMAND_ERROR_CATEGORY_GENERIC') });
            ...
            {
                await interaction.editReply({ components: [], content: `${localize(interaction.locale, 'UNBLOCK_COMMAND_ALL_CATEGORIES_NOT_BLOCKED')}` });
